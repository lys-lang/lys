(module
  ...
  (func $support::test::assert (param $condition i32)
    (call $support::test::assert_2 (local.get $condition) (i64.const 0x00000000000002af))
  )
  ...
  (func $test/fixtures/execution/stringifyNumbers.lys::assertNumberLength (param $numberToSerialize i64) (param $expected i32)
    ...
    (local $result i32)
    (block $B1
      (local.set $size (call $system::string::String.charLength (local.get $numberToSerialize)))
      (local.set $result (call $system::core::i32.== (call $system::core::u32.as_4 (local.get $size)) (local.get $expected)))
      (if $IF2 (call $system::core::boolean.! (local.get $result))
          (then
            (block $B3
                (call $support::env::printf_4 (i64.const 0x0000000000000010) (call $system::core::i64.as_4 (local.get $numberToSerialize)))
                (call $support::env::printf_4 (i64.const 0x0000000000000031) (call $system::core::u32.as_4 (local.get $size)))
                (call $support::env::printf_4 (i64.const 0x0000000000000052) (local.get $expected))
              )
          )
          (else)
        )
      (call $support::test::verify_12 (local.get $result) (i64.const 0x0000000000000073))
    )
  )
  (func $test/fixtures/execution/stringifyNumbers.lys::assertEquals_2 (param $given i64) (param $expected i64)
    ...
    (local $result i32)
    (block $B1
      (local.set $msg (call $system::string::concatBytes_1 (call $system::string::concatBytes_1 (call $system::string::concatBytes_1 (call $system::string::concatBytes_1 (i64.const 0x000000000000009c) (local.get $given)) (i64.const 0x00000000000000b1)) (local.get $expected)) (i64.const 0x00000000000000d0)))
      (local.set $result (call $system::core::bytes.== (local.get $given) (local.get $expected)))
      (if $IF2 (call $system::core::boolean.! (local.get $result))
          (then
            (block $B3
                (call $support::env::printf_8 (i64.const 0x00000000000000d7))
                (call $support::env::printf_8 (local.get $given))
                (call $support::env::printf_8 (i64.const 0x00000000000000e8))
                (call $support::env::printf_8 (local.get $expected))
              )
          )
          (else)
        )
      (call $support::test::verify_12 (local.get $result) (local.get $msg))
    )
  )
