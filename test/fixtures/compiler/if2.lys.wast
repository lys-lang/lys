  ...
  (func $test/fixtures/compiler/if2.lys::gcd (param $x i32) (param $y i32) (result i32)
    (block $B1 (result i32)
      (if $IF2 (result i32) (call $system::core::i32.> (local.get $x) (local.get $y))
          (then
            (block $B3 (result i32)
                (call $test/fixtures/compiler/if2.lys::gcd (call $system::core::i32.- (local.get $x) (local.get $y)) (local.get $y))
              )
          )
          (else
            (if $IF4 (result i32) (call $system::core::i32.< (local.get $x) (local.get $y))
                (then
                  (block $B5 (result i32)
                      (call $test/fixtures/compiler/if2.lys::gcd (local.get $x) (call $system::core::i32.- (local.get $y) (local.get $x)))
                    )
                )
                (else
                  (block $B6 (result i32)
