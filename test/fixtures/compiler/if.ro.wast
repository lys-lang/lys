(module
  ...
  (func $test::gcd (param $x i32) (param $y i32) (result i32)
    (if $a_wild_if (result i32) (call $system::core::> (get_local $x) (get_local $y))
      (then
        (call $test::gcd (call $system::core::- (get_local $x) (get_local $y)) (get_local $y))
      )
      (else
        (if $a_wild_if (result i32) (call $system::core::< (get_local $x) (get_local $y))
            (then
              (call $test::gcd (get_local $x) (call $system::core::- (get_local $y) (get_local $x)))
            )
            (else
              (get_local $x)
            )
  ...
  (func $test::ifWithoutElse (param $x i32) (result i32)
    ...
    (block $unknown_block_1 (result i32)
      ...
      (if $a_wild_if (call $system::core::==_11 (get_local $x) (i32.const 1))
          (then
