(module
  ...
  (func $test/fixtures/compiler/if.ro::gcd (param $x i32) (param $y i32) (result i32)
    (if $a_wild_if (result i32) (call $system::core::> (get_local $x) (get_local $y))
      (then
        (call $test/fixtures/compiler/if.ro::gcd (call $system::core::- (get_local $x) (get_local $y)) (get_local $y))
      )
      (else
        (if $a_wild_if (result i32) (call $system::core::< (get_local $x) (get_local $y))
            (then
              (call $test/fixtures/compiler/if.ro::gcd (get_local $x) (call $system::core::- (get_local $y) (get_local $x)))
            )
            (else
              (get_local $x)
            )
