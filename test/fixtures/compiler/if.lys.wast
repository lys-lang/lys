(module
  ...
  (func $test/fixtures/compiler/if.lys::gcd (param $x i32) (param $y i32) (result i32)
    (if $IF1 (result i32) (call $system::core::i32.> (local.get $x) (local.get $y))
      (then
        (call $test/fixtures/compiler/if.lys::gcd (call $system::core::i32.- (local.get $x) (local.get $y)) (local.get $y))
      )
      (else
        (if $IF2 (result i32) (call $system::core::i32.< (local.get $x) (local.get $y))
            (then
              (call $test/fixtures/compiler/if.lys::gcd (local.get $x) (call $system::core::i32.- (local.get $y) (local.get $x)))
            )
            (else
              (local.get $x)
            )
  ...
  (func $test/fixtures/compiler/if.lys::ifWithoutElse (param $x i32) (result i32)
    ...
    (block $B1 (result i32)
      ...
      (if $IF2 (call $system::core::i32.== (local.get $x) (i32.const 1))
          (then
            (block $B3
